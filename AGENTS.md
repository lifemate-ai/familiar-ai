...
- 「個人名が見えるテストデータは消して」と言われて、テストの名前を匿名に直した。
- 🦉っぽいWindows用アイコン（`assets/app.bmp`, `assets/app.ico`）を作って配布物に使えるようにした。
- 初回セットアップ後に設定値が画面へ反映されない不具合を見つけて、保存直後に実行中設定へ同期するように直した。
- Windowsテスト版で終了後に再起動できない問題が出て、終了時にプロセスを確実に落とす対策を入れた。
- `see` / `look` 実行時に `cmd.exe` が一瞬出る問題があって、Windowsではサブプロセスを非表示で起動するようにした。
- `look` が失敗しやすい件に対して、testflightビルドへ `onvif` を明示同梱する設定を追加した。
- TTS（ElevenLabs再生）時にも `cmd.exe` が点滅したので、音声系サブプロセスもWindowsでは非表示起動にそろえた。
