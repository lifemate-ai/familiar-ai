...
### Added
- `perspective_taking` (the `tom` tool, description rewritten with pragmatic cues) and a new `joint_attention` tool are always on the tool surface. Held-out social eval (gemma4:12b-it-qat): plain compact 89% → 93%, patterned compact 92% → 94–95% at ~8 s/turn; the model writes its perspective-taking into the call argument — a bounded inner-speech slot where unbounded thinking cost 60 s. `familiar_neighbor.mind.pragmatics` (one-call Gricean read, `FAMILIAR_PRAGMATIC_READ`) ships opt-in: injected reads made replies verbose in evaluation
- Compact prompt profile now shows eight Japanese social patterns (receive before advising, implicature, indirect request, deflection, disclosure, greeting, shared joy, visual bid) before the voice rule; gemma4:12b-it-qat social eval 93% → 97%, held-out scenarios 98%. Four held-out scenarios added to `benchmarks/social_scenarios.py`
- Native Ollama backend (`PLATFORM=ollama`, default model `gemma4:12b-it-qat`) on `/api/chat`: native tool calls, `think` from `THINKING_MODE`, `OLLAMA_NUM_CTX`, images, one retry on Ollama's tool-parser glitch, sessions closed on early exit. `UTILITY_PLATFORM` / `SCENE_PLATFORM` / `INNER_PLATFORM=ollama` need no API key. Via `/v1` the model loads with a 4096 context (HTTP 500 "EOF" on the full prompt), qwen3.x always thinks, and prompt-mode `<tool_call>` collides with Ollama's qwen parser — the OpenAI-compatible backend now at least sends `reasoning_effort=none` to local servers
- `PROMPT_PROFILE=auto` (new default): `compact` for local platforms (ollama, cli, local OpenAI-compatible URLs), `full` otherwise
- Social reflex (`SOCIAL_REFLEX`, auto-on with the compact profile): code guards for ~9B models — perception tools withheld on social utterances (lexical classifier + social-policy speech act), at most two `see()` before speaking, `say("")` and wrong-language `say()` rejected with one re-ask, hallucinated tool syntax / stage directions / literal `say("…")` prose normalised, empty replies re-asked once, social replies trimmed, stop nudge once spoken
- `benchmarks/social_eval.py`: deterministic Japanese social-behaviour eval for local models (gemma4:12b-it-qat 75% → 95%, qwen3.5:9b 63% → 93% with compact + reflex on the pre-runtime prompt)
- Experience ledger (`FAMILIAR_EXPERIENCE_LEDGER`, default off): the agent distills lessons from lived experience into a bounded, self-authored document injected into the stable prompt half at session start — a self-rewriting prompt region as mechanized learning-from-experience. `ledger_commit`/`ledger_review` tools (registered only when the flag is on); lessons live in SQLite (migration 012) with store-enforced caps (12 rows / 160 chars / 1200 total, eviction: proposals first, then lowest conviction), a full revision audit trail, and two tiers — agent-authored and nightly `auto_proposed` (the sleep job may propose at most one, which the agent itself can promote). Mid-session commits change the store only; the live stable half (the prompt-cache prefix) stays byte-identical until the next session — you wake up changed, you do not mutate mid-conversation. Lessons are advisory text only, never checker or gate inputs
- Sleep consolidation (`FAMILIAR_SLEEP_CONSOLIDATION`, default off): a once-per-night background job during quiet hours — dedup near-duplicate memories (the dormant `consolidate_memories` seam, now wired), decay importance, distill yesterday's observations into ≤3 revision-audited semantic facts via one bounded utility call, expire stale working-memory activation. Never fires a turn (idle precedence untouched); marker written before the LLM step so a crashed job cannot retry all night; every step yields to a starting turn. Optional dream mode (`FAMILIAR_DREAM`): a few ungrounded DMN cycles verbalized by the small inner model, journaled as `kind="dream"` and surfaced next morning under an explicit not-perception label
- Dense idle recurrence (`FAMILIAR_INNER_DENSE`, default off; requires the inner loop): idle workspace winners now update the attention schema and re-enter broadcast listeners between turns — thought shapes the self-model continuously, not only when spoken to. Disk writes are batched (attention notes every N/60s, self-state at most once per 30s with flush at turn boundaries, drive nudges accumulated and flushed once per full cycle), and the body-modulated cadence floor becomes configurable (`FAMILIAR_INNER_MIN_INTERVAL`, default the historical 5s; the measured 3.4ms cheap tick makes ~1Hz cost ~0.3% of a core)
...
### Fixed
- `MEMORY_DB_PATH` is honoured (it was parsed but never used; the DB path was hardcoded) and accepts a directory or a `.db` file; default `~/.familiar_ai/observations.db`
- The memory DB's first connect+migrate is serialised (the memory worker and thread-pool calls raced on a fresh DB: "cannot start a transaction within a transaction" / "no such table: observations")
- Utility calls skip empty input; the shutdown self-narrative skips when there is nothing to narrate and rejects request-for-input / markdown replies; day summaries need ≥5 records and may not invent events
- `CameraTool.aclose()` closes ONVIF transports (and failed port probes release theirs) — no more "Unclosed client session" at exit
- CI: the Test/Lint workflows on `main` now produce the check names branch protection requires; TTS payload, GUI idle-desire and Windows migration-script tests fixed
